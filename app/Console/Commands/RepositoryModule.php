...
class RepositoryModule extends Command
{
    ...
    public function handle()
    {
        ...
        $this->createRequests($name, $namespaceFolder);
        $this->createResource($name, $namespaceFolder);
    ...
    protected function createController(string $name, string $folder)
    {
        ...
        $storeRequest = "App\\Http\\Requests\\{$folder}\\{$name}\\Store{$name}Request";
        $updateRequest = "App\\Http\\Requests\\{$folder}\\{$name}\\Update{$name}Request";
        $resource = "App\\Http\\Resources\\{$folder}\\{$name}Resource";
...
class {$name}Controller extends Controller
{
    ...
    {
        return \$this->success(
            {$name}Resource::collection(\$this->service->getAll())
        );
    }
    ...
    public function store(Store{$name}Request \$request)
    {
        ...
        return \$this->created(new {$name}Resource(\$record));
    }

    public function show(int \$id)
    {
        return \$this->success(new {$name}Resource(\$this->service->findById(\$id)));
    }
    ...
    public function update(Update{$name}Request \$request, int \$id)
    {
        ...
        return \$this->success(new {$name}Resource(\$record));
    }
...
PHP;
    ...
    protected function createRequests(string $name, string $folder)
    {
        $dir = app_path("Http/Requests/{$folder}/{$name}");
        File::ensureDirectoryExists($dir);

        foreach (['Store', 'Update'] as $prefix) {
            $path = "{$dir}/{$prefix}{$name}Request.php";
            $namespace = "App\Http\Requests\\{$folder}\\{$name}";
...
PHP;
            ...
            $this->createFile($path, $content, "{$prefix} Request");
        }
    }

    protected function createResource(string $name, string $folder)
    {
        $dir = app_path("Http/Resources/{$folder}");
        File::ensureDirectoryExists($dir);
...
PHP;
        ...
        $this->createFile($path, $content, 'Resource');
    }
